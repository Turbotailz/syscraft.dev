...
const EDIT_REPO = 'https://github.com/Turbotailz/syscraft.dev'
...
function originalFileUpdatedAt(filePath?: string): string | undefined {
  ...
  const originalRel = originalWikiPath(contentRelative(filePath))
  if (!originalDir || !originalRel) {
    return
  }
  try {
    const date = execFileSync('git', ['log', '-1', '--pretty=format:%cI', '--', originalRel], {
      cwd: originalDir,
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore']
    }).trim()
...
function contributorsForFile(repoRoot: string, rel: string): ContentContributor[] {
  ...
  return mergePeople(
    gitAuthorsForFile(repoRoot, rel),
    originalDir && originalRel ? gitAuthorsForFile(originalDir, originalRel) : []
  )
}
